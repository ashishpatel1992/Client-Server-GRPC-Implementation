...
class AddRPCServiceClient {
 public:
  ...
  // from the server.
  std::string Call(const std::string& user, const std::string& data) {
...
int main(int argc, char* argv[]) {
  ...
  AddRPCServiceClient AddRPCService(grpc::CreateChannel("localhost:50051", grpc::InsecureChannelCredentials()));
  std::string user;
  user = argv[1];
  std::string args;
